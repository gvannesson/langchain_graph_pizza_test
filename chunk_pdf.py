...
PDF_FILE = ["Menu.pdf", "Liste_allergenes.pdf"]  # Le chemin est relatif au script
...
def load_and_chunk_pdf(file_path, chunk_size=200, chunk_overlap=50):
    """
...
for pdf in PDF_FILE:
    pdf_chunks = load_and_chunk_pdf(pdf)
    collection.add(documents=pdf_chunks, ids=[f"chunk_{pdf}_{i}" for i in range(len(pdf_chunks))])
